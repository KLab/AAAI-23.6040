...
if __name__ == '__main__':
    ...
    for pack_name, pack_sm_glob in zip(pack_names, pack_sm_globs):
        ...
        for sm_fp in pack_sm_fps:
            ...
            # extract time signatures from ssc file if exists
            ssc_file_path = os.path.join(Path(sm_fp).parent,
                                         Path(sm_fp).stem + ".ssc")
            time_signatures = None
            if Path(ssc_file_path).exists():
                with open(ssc_file_path, 'r') as ssc_file:
                    time_signatures = extract_time_signature(ssc_file.read())[
                        'timesignatures']
                    if len(time_signatures) > 1:
                        print(f"time signature len: {len(time_signatures)}")

            bpm_info = calc_bpm_info(offset, bpms, stops, time_signatures)
